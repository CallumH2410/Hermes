...
# ───────────────────────────── baseline RTT (reflect) ───────────────────────
def cmd_rtt(a):
    """Single-clock forwarding-latency baseline. Requires baseline_fwd.p4 in
    REFLECT mode (run_baseline_bfrt.py --mode reflect), so a frame sent from h1
    is bounced back to h1. We timestamp send and matching receive on one clock
    and report the round trip. Writes one row PER SAMPLE -> enables true
    boxplots in plot_eval.py. One-way forwarding latency is ~RTT/2."""
    s = socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(0x0003))
    s.bind((a.iface, 0))
    try:
        SOL_PACKET = getattr(socket, "SOL_PACKET", 263)
        ADD = getattr(socket, "PACKET_ADD_MEMBERSHIP", 1)
        mreq = struct.pack("iHH8s", socket.if_nametoindex(a.iface), 1, 0, b"")
        s.setsockopt(SOL_PACKET, ADD, mreq)               # promiscuous: dst MAC != ours
    except OSError as e:
        print(f"[rtt] WARNING: promisc not enabled ({e}); "
              f"run 'sudo ip link set {a.iface} promisc on'")
    s.settimeout(a.timeout)

    samples = []
    sent = lost = 0
    for i in range(a.count):
        frame = build_frame(a.flow_id, i + 1, logical_flow=0xFFFF,
                            pkt_idx=i, size=a.size)
        ...
        while time.perf_counter() < deadline:
            try:
                ...
                break
            if len(addr) >= 3 and addr[2] == socket.PACKET_OUTGOING:
                continue                                   # skip our own outgoing copy
            if len(pkt) < HDR_LEN + TAG_LEN:
                continue
            if pkt[12:14] != struct.pack("!H", ETHERTYPE_HERMES) or pkt[14] != MSG_DATA:
                continue
            magic, lflow, pidx = struct.unpack("!III", pkt[HDR_LEN:HDR_LEN + TAG_LEN])
            if magic == TAG_MAGIC and pidx == i:
    ...
    if samples:
        arr = sorted(samples)
        import statistics
        p = lambda q: arr[min(len(arr) - 1, int(q * len(arr)))]
        print(f"[rtt] n={len(arr)} lost={lost}  RTT ms: "
              f"min={arr[0]:.3f} p50={statistics.median(arr):.3f} "
              f"p95={p(0.95):.3f} max={arr[-1]:.3f}  (one-way ~ RTT/2)")
    else:
        print(f"[rtt] no replies (sent={sent}, lost={lost}). Check REFLECT mode is "
              f"installed and {a.iface} is promiscuous.")
    with open(a.out, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["sample_idx", "rtt_ms"])
        for idx, v in enumerate(samples):
            w.writerow([idx, f"{v:.6f}"])
    print(f"[rtt] wrote {len(samples)} samples to {a.out}")
...
def main():
    ...
    prt = sub.add_parser("rtt", help="baseline forwarding RTT (reflect mode)")
    prt.add_argument("--iface", required=True)
    prt.add_argument("--flow-id", type=int, default=1, dest="flow_id")
    prt.add_argument("--size", type=int, default=64)
    prt.add_argument("--count", type=int, default=1000)
    prt.add_argument("--gap", type=float, default=0.002, help="inter-sample gap (s)")
    prt.add_argument("--timeout", type=float, default=0.5)
    prt.add_argument("--out", default="rtt_baseline.csv")
    prt.set_defaults(func=cmd_rtt)
