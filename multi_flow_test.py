...
def simulate_packet(flow_id, switch_states):
    path = FLOWS[flow_id]
    acc = 0

    print(f"\nSimulating flow {flow_id}: {' -> '.join(path)}")
...
def send_verify(flow_id, acc_final, seq):
    timestamp_ms = int(time.time() * 1000)
    nonce = random.randint(1, 2**63 - 1)
    hop_count = len(FLOWS[flow_id])
    ...
    print(f"\n>>> VERIFY flow={flow_id}, seq={seq}, acc=0x{acc_final:08x}")
    resp = send_line(msg)
    print(f"<<< {resp}")
...
def main():
    ...
    print("\n==============================")
    print("Starting multi-flow verification")
    print("==============================")

    for flow_id, seq in tests:
        acc_final = simulate_packet(flow_id, switch_states)
        resp = send_verify(flow_id, acc_final, seq)
